...
var compareVersion = function (version1, version2) {
  ...
  // 想法1: 直接去掉点,补位后当做整数来比较
  // 不可行... '1.01.01', '1.001.1'


  // 想法2, .分隔, 逐个比较, 比大小不相等直接退出
  let res = 0
  const v1 = version1.split('.')
  const v2 = version2.split('.')
  for (let i = 0, l = Math.max(v1.length, v2.length); i < l; i++) {
    const res = (Number(v1[i]) || 0) - (Number(v2[i]) || 0)
    if (res) {
...
test(compareVersion, ['1.01.01', '1.001.1'], 0)
